...
function App() {
  ...
  return <div className={`jarvis-app ${isDemo ? 'demo-mode' : 'personal-mode'}`}>
    <aside className="side-rail">
      ...
      <nav>{activeNav.map(([id, icon, label]) => <button key={id} className={page === id ? 'active' : ''} onClick={() => setPage(id)}><i>{icon}</i><span>{label}</span>{id === 'actions' && approvals.length > 0 && <em>{approvals.length}</em>}</button>)}</nav>
      <div className="rail-card"><span>{isDemo ? 'DEMO PROFILE' : 'PROFILE ENGINE'}</span><strong>{profile.resume_connected ? 'Resume online' : 'Resume needed'}</strong><small>{profile.skills?.length || 0} skills mapped</small><label aria-disabled={isDemo}>{isDemo ? 'Read-only demo' : profile.resume_connected ? 'Update resume' : 'Add resume'}{!isDemo && <input type="file" accept=".pdf,.txt" onChange={uploadResume} />}</label></div>
    ...
    <main className="product-area">
      ...
      {page === 'insights' && <section className="page">
        ...
        <div className="insight-grid">
          <form className="composer settings-form" onSubmit={saveSettings}><span>CAREER PREFERENCES</span><h2>Calibrate JARVIS</h2><input value={settings.display_name} onChange={(e) => setSettings({...settings, display_name:e.target.value})} placeholder="Display name" required disabled={isDemo}/><input value={settings.target_role} onChange={(e) => setSettings({...settings, target_role:e.target.value})} placeholder="Target role" required disabled={isDemo}/><input value={settings.target_location} onChange={(e) => setSettings({...settings, target_location:e.target.value})} placeholder="Target location" required disabled={isDemo}/><label>Minimum profile fit<input type="number" min="0" max="100" value={settings.minimum_fit} onChange={(e) => setSettings({...settings, minimum_fit:Number(e.target.value)})} disabled={isDemo}/></label><label className="check"><input type="checkbox" checked={settings.weekly_digest} onChange={(e) => setSettings({...settings, weekly_digest:e.target.checked})} disabled={isDemo}/> Weekly intelligence digest</label><label className="check"><input type="checkbox" checked={settings.follow_up_reminders} onChange={(e) => setSettings({...settings, follow_up_reminders:e.target.checked})} disabled={isDemo}/> Follow-up reminders</label><button disabled={isDemo}>{isDemo ? 'Read-only demo' : 'Save preferences ↗'}</button></form>
          <section className="panel weekly-panel"><header><div><span>WEEKLY COMMAND BRIEF</span><h2>Momentum at a glance</h2></div></header>{weekly && <div className="weekly-stats"><article><strong>{weekly.new_applications}</strong><small>NEW APPLICATIONS</small></article><article><strong>{weekly.interviews_moved}</strong><small>INTERVIEWS</small></article><article><strong>{weekly.interview_rate}%</strong><small>INTERVIEW RATE</small></article><p>{weekly.recommended_focus}</p></div>}<div className="funnel" aria-label="Application funnel">{(analytics.funnel || stages.map((stage) => ({stage,total:pipeline[stage].length}))).map((item) => <div key={item.stage}><span>{item.stage}</span><i><b style={{width:`${Math.max(6, (item.total / Math.max(1, applications.length)) * 100)}%`}} /></i><strong>{item.total}</strong></div>)}</div><div className="notification-list">{notifications.map((item) => <article key={item.id}><b>{item.title}</b><small>{item.detail}</small></article>)}{!notifications.length && <p>All clear. No reminders need attention.</p>}</div></section>
        </div>
      ...
      {page === 'showcase' && <section className="page showcase-page"><PageHeader kicker="ENGINEERING CASE STUDY" title="Built as a career operating system." copy="A production-deployed, approval-gated intelligence product—not a static dashboard." action={<a className="primary" href="https://github.com/syedsaud15/jarvis-career-os" target="_blank" rel="noreferrer">View source ↗</a>}/><div className="showcase-grid"><article><span>01 · DISCOVER</span><h2>Market intelligence</h2><p>Live job discovery, explainable ranking, quality scoring, deduplication and India-wide filters.</p></article><article><span>02 · DECIDE</span><h2>Career copilot</h2><p>Resume evidence, skill gaps, tailored applications and role-specific interview preparation.</p></article><article><span>03 · EXECUTE</span><h2>Human-in-the-loop automation</h2><p>Every external Gmail or Calendar action requires explicit approval and creates an audit record.</p></article><article><span>04 · OPERATE</span><h2>Production engineering</h2><p>FastAPI, React, PostgreSQL, Docker, CI, OAuth, health checks, validation and secure deployment.</p></article></div><section className="architecture"><span>REFERENCE ARCHITECTURE</span><div><b>React experience</b><i>→</i><b>NGINX security edge</b><i>→</i><b>FastAPI services</b><i>→</i><b>Neon PostgreSQL</b></div><small>External providers: Adzuna · Google OAuth · Gmail · Calendar</small></section></section>}
